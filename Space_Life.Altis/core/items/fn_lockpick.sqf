...
_title = format[localize "STR_ISTR_Rob_Car",if(!_isVehicle) then {"Handcuffs"} else {getText(configFile >> "CfgVehicles" >> (typeOf _curTarget) >> "displayName")}];
life_action_inUse = true; //Lock out other actions[[_curTarget],"life_fnc_CarAlarmSound",nil,true] spawn life_fnc_MP;
...
_title = format[localize "STR_ISTR_Rob_Car",if(!_isVehicle) then {"Handcuffs"} else {getText(configFile >> "CfgVehicles" >> (typeOf _curTarget) >> "displayName")}];life_action_inUse = true; //Lock out other actions[[_curTarget],"life_fnc_CarAlarmSound",nil,true] spawn life_fnc_MP;
